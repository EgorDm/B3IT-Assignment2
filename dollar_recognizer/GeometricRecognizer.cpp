...
namespace DollarRecognizer {
    ...
    double GeometricRecognizer::distanceAtAngle(
        ...
        Path2D newPoints = rotateBy(std::move(points), rotation);
    ...
    */
    ...
    void GeometricRecognizer::activateMultiStrokesTemplates(vector<string> list) {
        ...
        for (unsigned int i = 0; i < allMtemplates.size(); i++) {
            ...
            if (inTemplates(allMtemplates.at(i).name, list)) {
                ...
                MultiStrokeGesture unistrokes = GeometricRecognizer::MakeUnistrokes(
                        strokes.paths); // returns array of point arrays
                allunistrokenormalizedtemplates.clear();
                for (int j = 0; j < unistrokes.size(); j++)
                    Path2D unistroke = GeometricRecognizer::UnistrokeTemplate(unistrokes.at(j),
                                                                              allMtemplates.at(i).name);
    ...
    Point2D GeometricRecognizer::CalcStartUnitVector(Path2D points,
                                                     double index) // start angle from points[0] to points[index] normalized as a unit vector
    {
    ...
    RecognitionResult GeometricRecognizer::Multirecognize(MultiStrokeGesture strokes, string method) {
        ...
        for (int i = 0; i < (int) allmultistrokenormalizedgestures.size(); i++) // for each multistroke
        {
            ...
            if (!requireSameNoOfStrokes ||
                strokes.size() == Mgestures.size()) // optional -- only attempt match when same # of component strokes
            {
                for (int j = 0; j < Mgestures.size(); j++) // each unistroke within this multistroke
                {
                    GestureTemplate Mgesture = Mgestures.at(j);
                    if (AngleBetweenUnitVectors(startv, Mgesture.startv) <=
                        AngleSimilarityThreshold) // strokes start in the same direction
                    {
                        double distance;
    ...
    double GeometricRecognizer::AngleBetweenUnitVectors(Point2D v1,
                                                        Point2D v2) // gives acute angle between unit vectors from (0,0) to v1, and (0,0) to v2
    {
